...
input_smile = pd.read_csv(f'{path}/input1.csv')
test_pred["prob"] = test_pred["property"].apply(
    lambda x: x if x > 0.5 else (1 - x))
input_smile["prob"] = test_pred["prob"]
input_smile["pred_odor"] = test_pred["property"].round(0)
# input_smile.sort_values("prob",ascending=False,inplace=True)
...
predicted_output.rename(columns={'smiles': 'SMILES'}, inplace=True)
